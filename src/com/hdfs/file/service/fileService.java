...
public interface fileService {
	...
	Boolean rootmkdir(long parentId, String name, long userId, long rootid)
			throws IOException;
	 ...
	 * @return
	 */
	dillResult uploadFile(long currentId, File file, String filename,
			HdfsMemory memory, int safelevel, String deadline);

	boolean isPublicKeyEmpty(Users user);

	/**
	 * 第一次使用加密上传文件功能时： 生成rsa密钥对，并保存公钥的内容到数据库中。
	 * 
	 * @param user
	 */
